...
/*
    GameSparks LLC:
        ...
        Compatibility with Marmalade (https://www.madewithmarmalade.com/)
        was ensured by using gethostbyname() for name resolution.
*/
...
#	include <netdb.h>
#else
//#	define read(fd, buf, len)	sceNetRecv(fd, buf, len, 0)
//#	define write(fd, buf, len)	sceNetSend(fd, buf, len, 0)
...
#else
#if !defined(EFIX64) && !defined(EFI32) && !defined(__ORBIS__)
...
*/
#if defined(MARMALADE)
// marmalade versions prior 7.10 do not implement getaddrinfo.
int mbedtls_net_connect(mbedtls_net_context *ctx, const char *host, const char *port, int proto)
{
	...
	addr_in.sin_addr = *((struct in_addr *)server_hostent->h_addr);
...
/// if you get an compile error about a missing include here,
/// you have not put the PS4 support files into the currect location.
/// please copy mbed_tls_net_ps4.inc into src/mbedtls.
/// If you don't have this file, please contact GameSaprks support.
#include "./mbed_tls_net_ps4.inc"
#else
int mbedtls_net_connect(mbedtls_net_context *ctx, const char *host, const char *port, int proto)
{
	int ret;
